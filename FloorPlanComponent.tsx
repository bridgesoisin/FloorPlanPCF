...
// ── App palette (used in canvas draw calls that can't use CSS vars) ────────────
const TEAL   = '#176d82';   // primary accent — matches app buttons
const NAVY   = '#113257';   // header background
const TEAL_D = '#0f5060';   // darker teal for hover / outline
const TEAL_L = '#a8d4e0';   // light teal for in-progress measure text
...
const SignaturePadComponent: React.FC<Props> = ({ value, sketchName, onSave }) => {
    ...
    function drawWallPoly(c:CanvasRenderingContext2D, pts:Pt[]) {
        ...
        if(n>2){
            ...
            c.closePath();c.fillStyle='#f0f4f8';c.fill();
        }
        c.save();
        ...
        pts.forEach((p,i)=>{
            ...
            c.fillStyle=i===0?TEAL:'#1d2230';c.fill();
    ...
    function drawWallInProgress(c:CanvasRenderingContext2D, pts:Pt[], ghost:Pt|null) {
        const n=pts.length;
        c.save();c.strokeStyle='#2a5068';c.lineWidth=3;c.lineCap='square';c.setLineDash([8,5]);
        c.beginPath();c.moveTo(gp(pts[0].x),gp(pts[0].y));
        for(let i=1;i<n;i++) c.lineTo(gp(pts[i].x),gp(pts[i].y));
        ...
        if(ghost){
            const last=pts[n-1];
            c.save();c.strokeStyle=TEAL;c.lineWidth=2;c.setLineDash([5,4]);
        ...
        pts.forEach((p,i)=>{
            c.beginPath();c.arc(gp(p.x),gp(p.y),i===0?8:5,0,Math.PI*2);
            c.fillStyle=i===0?TEAL:'#2a5068';c.fill();
            if(i===0){c.strokeStyle='white';c.lineWidth=2;c.stroke();}
        });
    }
    ...
    function drawMeasure(c:CanvasRenderingContext2D, a:Pt, b:Pt, inProg:boolean) {
        ...
        // In-progress: use navy pill; completed: dark pill — both readable on light canvas
        c.fillStyle=inProg?`rgba(${NAVY.replace('#','').match(/../g)!.map(x=>parseInt(x,16)).join(',')},0.90)`:'rgba(15,34,55,.88)';
        rrect(c,mx+nx-tw/2,my+ny-13,tw,26,5);c.fill();
        c.fillStyle=inProg?TEAL_L:'#ffffff';
    ...
    function drawFixture(c:CanvasRenderingContext2D, f:Fixture, selected:boolean) {
        ...
        if(selected){
            c.strokeStyle=TEAL;c.lineWidth=2.5;c.setLineDash([5,4]);
    ...
    function drawHandles(c:CanvasRenderingContext2D, f:Fixture) {
        ...
        c.save();c.translate(cx,cy);c.rotate(f.rotation||0);
        HANDLES.forEach(h=>{
            c.fillStyle='white';c.strokeStyle=TEAL;c.lineWidth=1.5;
            c.fillRect(h.fx*pw-6,h.fy*ph-6,12,12);c.strokeRect(h.fx*pw-6,h.fy*ph-6,12,12);
        });
        // Rotate handle arm + circle — teal to match accent
        c.strokeStyle=TEAL;c.lineWidth=1.5;
        c.beginPath();c.moveTo(0,-ph/2);c.lineTo(0,-ph/2-28);c.stroke();
        c.beginPath();c.arc(0,-ph/2-28,7,0,Math.PI*2);
        c.fillStyle=TEAL;c.fill();c.strokeStyle=TEAL_D;c.lineWidth=1;c.stroke();
        c.restore();
    }
    ...
    function doSetTool(t:string){
        ...
        const msgs:Record<string,string>={
            select:'Click to select · Drag to move · Corner handles resize · Teal ● rotates',
            wall:  'Tap to add wall points · Tap the teal ● to close the room',
        ...
        function renderScene(sceneKey:'before'|'after'): string {
            ...
            if(n>0){
                if(n>2){
                    ...
                    oc.closePath();oc.fillStyle='#f0f4f8';oc.fill();
                }
            ...
            oc.fillStyle='rgba(17,50,87,.45)';oc.textAlign='right';
            oc.fillText(
                (sketchNameRef.current||'Classic Bathrooms')+' · '+sceneKey.toUpperCase()+' · 1 sq=200mm',
    ...
    function onMove(e:PointerEvent){
        ...
        if(tool==='wall'){
            ...
            if(pts.length>0){
                ...
                if(mm>0) setStatusMsg('Drawing … '+mm+'mm · Tap teal ● to close room');
            }
            render(); return;
        }
    ...
    function doResize(f:Fixture, pos:{x:number;y:number}){
        ...
        const co = Math.cos(rot), si = Math.sin(rot);
        const alx = -h.fx * s.w * G, aly = -h.fy * s.h * G;
        const awx = ocx + alx*co - aly*si, awy = ocy + alx*si + aly*co;
        const nalx = -h.fx * nw * G, naly = -h.fy * nh * G;
        const ncx = awx - (nalx*co - naly*si), ncy = awy - (nalx*si + naly*co);
        f.w = nw; f.h = nh; f.x = ncx/G - nw/2; f.y = ncy/G - nh/2;
    }
    ...
    React.useEffect(()=>{
        const canvas=canvasRef.current; if(!canvas) return;
        if(value){
            try{ const saved=JSON.parse(value); if(saved.scenes) scenesRef.current=saved.scenes; }
            catch{/* ignore */}
        }
    ...
    return (
        <div style={{width:'100%',height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}}>
            <style>{`
                /* ── App palette ───────────────────────────────────────────
                   --hdr    #113257  RGBA(17,50,87,1)  — exact app header
                   --accent #176d82                    — app teal buttons
                   --bg     #f0f3f7                    — light page bg
                   --panel  #ffffff                    — white sidebar/footer
                   --border #cdd5df                    — light dividers
                   --text   #0f2233                    — dark navy body text
                   --muted  #607080                    — secondary text
                ─────────────────────────────────────────────────────────── */
                .bpc{
                    --hdr:#113257; --accent:#176d82; --accent-d:#0f5060;
                    --bg:#f0f3f7; --panel:#ffffff; --border:#cdd5df;
                    --text:#0f2233; --muted:#607080;
                    --red:#d94f4f; --amber:#e8a020;
                    font-family:'Segoe UI',system-ui,-apple-system,sans-serif;
                    color:var(--text);background:var(--bg);
                    width:100%;height:100%;display:flex;flex-direction:column;overflow:hidden;
                }

                /* Header — exact app navy */
                .bpc-hdr{
                    height:52px;background:var(--hdr);
                    border-bottom:2px solid var(--accent);
                    display:flex;align-items:center;padding:0 16px;gap:12px;flex-shrink:0;
                }
                .bpc-logo{display:flex;flex-direction:column;align-items:flex-start;line-height:1.15;white-space:nowrap;}
                .bpc-logo-top{font-size:11px;font-weight:400;color:rgba(255,255,255,.7);letter-spacing:.5px;text-transform:uppercase;}
                .bpc-logo-bot{font-size:14px;font-weight:700;color:#ffffff;letter-spacing:-.2px;}
                .bpc-div{width:1px;height:24px;background:rgba(255,255,255,.2);}

                /* Before / After tabs — on header */
                .bpc-tab{
                ...
                .bpc-tab:hover{border-color:rgba(255,255,255,.6);color:#fff;}
                ...
                /* Status message */
                .bpc-status{
                    margin-left:auto;font-size:12px;color:rgba(255,255,255,.6);
                ...
                /* Sidebar — white card */
                .bpc-side{
                    width:164px;background:var(--panel);
                    border-right:1px solid var(--border);
                    display:flex;flex-direction:column;overflow-y:auto;flex-shrink:0;
                    padding-bottom:12px;
                }
                .bpc-sec{
                    padding:10px 10px 4px;font-size:9px;letter-spacing:1.5px;
                ...
                .bpc-tb{
                    ...
                    width:calc(100% - 14px);margin:0 7px 2px;
                    padding:8px 10px;background:transparent;
                    border:1.5px solid transparent;border-radius:6px;
                    color:var(--text);font-family:inherit;font-size:12px;
                    cursor:pointer;text-align:left;transition:all .12s;
                }
                .bpc-tb:hover{background:#eef2f6;border-color:var(--border);}
                .bpc-tb.on{background:rgba(23,109,130,.10);border-color:var(--accent);color:var(--accent);}
                .bpc-tb.red{color:var(--red);}
                .bpc-tb.red:hover{background:rgba(217,79,79,.08);border-color:var(--red);}
                .bpc-tb.redon{background:rgba(217,79,79,.12);border-color:var(--red);color:var(--red);}
                ...
                .bpc-fb:hover{border-color:var(--accent);color:var(--text);background:rgba(23,109,130,.06);}
                .bpc-fb.pl{border-color:var(--accent);color:var(--accent);background:rgba(23,109,130,.10);}

                /* Canvas area */
                .bpc-wrap{flex:1;overflow:auto;background:#e6eaf0;padding:24px;}
                .bpc-canvas{display:block;background:#f8f9fb;touch-action:none;
                    box-shadow:0 1px 4px rgba(17,50,87,.12);}
                ...
                .bpc-save:hover{background:var(--accent-d);}
            ...
            <div className="bpc">
                ...
                <div className="bpc-hdr">
                    <div className="bpc-logo">
                        <span className="bpc-logo-top">Classic</span>
                        <span className="bpc-logo-bot">Bathrooms</span>
                    </div>
                ...
                <div className="bpc-body">
                    ...
                    <div className="bpc-side">
                        ...
                        <div className="bpc-sec" style={{marginTop:6}}>Fixtures</div>
                ...
                <div className="bpc-ftr">
                    <button className="bpc-save" onClick={doSave}>☁ Save to Survey</button>
                    <span className="bpc-hint">Ctrl+Z undo · ESC cancel · DEL delete · Teal ● rotate</span>
